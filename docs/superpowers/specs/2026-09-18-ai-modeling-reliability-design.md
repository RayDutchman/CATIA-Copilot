...
- **日期**：2026-09-18
- **状态**：架构方向已批准，分阶段规格与 S1 精细计划已落盘，待书面复核
...
| 阶段 | 名称 | 主题 | 状态 |
|---|---|---|---|
| S1 | 能力契约统一 | 单一来源建模描述 + 契约测试 | 本期第一优先 |
| S2 | 几何查询可靠性 | planar/cylindrical/unknown 分类，不伪造法向 | 未启动 |
| S3 | 文档绑定与运行记录 | 目标文档绑定、每 run 记录、状态分层 | 未启动 |
| S4 | 无 CATIA 测试与手动基准 | 单元测试 + 有限手动基准 | 未启动 |
...
- **验收**：无 CATIA 环境单元测试全绿；手动基准逐项有结果记录；断言均以几何/属性为准而非仅特征名。
- **stop**：若手动基准暴露需改 COM 实现的缺口，回退补 S2/S3 再进。

## 8. 阶段进入规则与实施纪律

- 每阶段进入前单独写精细计划到 `docs/superpowers/plans/`；本次只落 S1 计划（另一 agent 写），后阶段进入时再各写一份。
...
| S4 | S1-S3 纯逻辑可 mock；手动基准清单与 CATIA 环境就绪 |
