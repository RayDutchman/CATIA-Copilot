...
class _BomTreeWidget(QTreeWidget):
    ...
    重写 :meth:`drawRow`：当 QComboBox 等子控件弹出导致树失去焦点时，Qt 会将
    已选行的 State_Active 清除，导致 item:selected:!active 规则生效而变色。
    通过在绘制前强制补回 State_Active 标志，使选中行始终以激活色绘制。
    """
