...
**Interfaces:**
- Consumes: `catia_copilot.plm.my_pdm_schemas` 中的 TokenResponse, UserResponse, BomMatchResult, BomSyncResponse, CadNamingConfig, AttachmentResponse, PartCreateRequest, PartCreateResponse, has_permission
- Produces: `MyPdmApiClient` 类 和 `MyPdmApiError` 异常类
...
**验证:** `python -c "from catia_copilot.plm.my_pdm_api_client import MyPdmApiClient; c = MyPdmApiClient('http://localhost/api'); print('client created OK'); print('has_permission:', c.can('parts:read'))"` → 输出 `client created OK` 和 `has_permission: False`
